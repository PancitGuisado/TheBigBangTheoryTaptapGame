const fs = require('fs');
let vectors = fs.readFileSync('vectors.js', 'utf8');
...
// Insert the new SVGs before the closing brace of the vectors object
const insertIdx = vectors.lastIndexOf('};');
if (insertIdx !== -1) {
    vectors = vectors.slice(0, insertIdx) + ',\n' + roombaSVG + '\n' + droneSVG + '\n' + vectors.slice(insertIdx);
    // Fix trailing comma if needed
    vectors = vectors.replace(/,(\s*,\s*\"roomba_doom\")/, '$1');
    fs.writeFileSync('vectors.js', vectors);
    console.log("Added bot SVGs to vectors.js");
} else {
    console.log("Could not find end of vectors object.");
}
