...
function spawnEnemy() {
    ...
    if (window.manualBossTrigger) {
        ...
        bossTimer = 20.0;
        if (timerDisplay) timerDisplay.innerText = "[TIME] " + bossTimer.toFixed(1) + "s";
    ...
    for (const [key, config] of Object.entries(characters)) {
        ...
        gameTimers[key] = setInterval(() => {
            ...
            if (key === 'penny') {
                ...
                const healAmt = 8 + (state.roster[key].level * 2); // 10 at lvl 1, +2 per upgrade
                let healed = false;
                Object.keys(state.equipped).forEach(charKey => {
                    if (state.equipped[charKey]) {
                        const charData = state.roster[charKey];
                        const charConfig = characters[charKey];
                        const maxHp = charData.maxHp || Math.floor((charConfig.baseHp || 100) * Math.pow(charConfig.lane === 'front' ? 1.80 : 1.25, charData.level - 1));
                ...
                if (key === 'sheldon' && typeof sheldonTapBuff !== 'undefined') outDmg = Math.floor(outDmg * (1 + sheldonTapBuff));
                processDamage(outDmg, key);
            }
        }, rate);
    }
...
function generateDamagePopup(event, val, isCrit, isSpecialText, isEnemyDamage = false) {
    ...
    setTimeout(() => pop.remove(), isSpecialText ? 1500 : 800);
}
                ...
                if (robot && robot.equipped && robot.overheated) {
                    ...
                    repairContainer.innerHTML += `
                        <div onclick="if(${canRepair}) executeRobotRepair('${robot.blueprintId}')" class="px-2 py-1 rounded border border-amber-700 shadow-md flex items-center gap-1 text-[8px] font-bold uppercase ${btnClass} transition-colors pointer-events-auto">
                            <span class="text-lg">🔧</span> REPAIR ${botName} (${repairCost} SCRAP)
                        </div>
                    `;
                }
            });
        ...
        lanes[laneKey].forEach((char, index) => {
            ...
            container.innerHTML += `
                <div id="live-character-${key}" 
                     ...
                     style="z-index: ${20 + index}; ${hangoutStyle}">
                    ${hpBarHtml}
                    ${trackingBadge}
...
function openModal(event, key) {
    ...
    const cHp = Math.floor((config.baseHp || 100) * Math.pow(config.lane === 'front' ? 1.80 : 1.25, currLvl - 1));
    const cCd = Math.max(500, (config.atkSpeed || 2000) * Math.pow(0.96, currLvl - 1));
    
    const nDmg = config.baseDmg * nextLvl;
    const nHp = Math.floor((config.baseHp || 100) * Math.pow(config.lane === 'front' ? 1.80 : 1.25, nextLvl - 1));
    ...
    if (passiveEl) {
        if (!config.passiveType) {
            ...
            if (config.passiveType === 'heal') {
                ...
                unit = 's';
            } else if (config.passiveType === 'coolDown') {
    ...
    if (mode === 'buy') {
        if (state.resources.money >= currentCost) {
            ...
            if (!state.roster[activeModalKey]) {
                ...
                const oldMaxHp = Math.floor((config.baseHp || 100) * Math.pow(config.lane === 'front' ? 1.80 : 1.25, state.roster[activeModalKey].level - 1));
                state.roster[activeModalKey].level++;
                const newMaxHp = Math.floor((config.baseHp || 100) * Math.pow(config.lane === 'front' ? 1.80 : 1.25, state.roster[activeModalKey].level - 1));
...
window.executeRobotRepair = function(key) {
    ...
    if (state.resources.scrap >= repairCost) {
        ...
        if (theRobot) {
            ...
            if (typeof renderRobotBattleLine === 'function') renderRobotBattleLine();
            const rModal = document.getElementById('robot-modal');
