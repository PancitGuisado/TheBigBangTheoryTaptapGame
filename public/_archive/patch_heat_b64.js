...
let robotAutoReplacement = `       window.robotTimers[robot.id] = setInterval(() => {
			if (robot.overheated) return;
			...
			robot.heat += Math.max(1, Math.floor(robot.maxHeat * 0.05));
...
const renderRobotLineReplacement = `                   <!-- Robot Heat Bar -->
						${(() => {
								...
								const hpBarHtml = \\`
								<div class="absolute -top-3 left-0 right-0 h-1.5 bg-gray-900 border border-gray-700 rounded-sm overflow-hidden z-40 opacity-90 shadou-[0_0_5px_rgba(255,100,0,0.5)] pointer-events-none">
...
if (appV2.includes(renderRobotLineStart)) {
    appV2 = appV2.replace(renderRobotLineStart, renderRobotLineReplacement);
    appV2 = appV2.replace(/<div class="character-vector-wrapper flex items-center justify-center text-4xl " style="/g, '<div class="character-vector-wrapper flex items-center justify-center text-4xl ${robot.overheated ? \\'grayscale brightness-50 sepia blur-[1px]\\' : \\'\v}" style="');
}
...
if (appV2.includes(howardAttack)) {
    appV2 = appV2.replace(howardAttack, howardReplacement);
}

// 4. ExecuteRepair
if (!appV2.includes('executeRobotRepair')) {
...
fs.writeFileSync('app_v2.js', appV2);
require('acorn').parse(appV2, { ecmaVersion: 2022 });
console.log('Heat Patch Applied Successfully');
